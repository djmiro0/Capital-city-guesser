...
describe('Add ToDos', () => {
    ...
    it("Page should contain 'Create Task' button", async () => {
        const button = await page.$x("//*[contains(translate(text(), 'CREATE', 'create'),'create')]")
...
describe('Delete ToDos', () => {
    it("Items should have 'x' delete button", async () => {
        ...
        const button = await page.$x("//*[contains(translate(text(), 'CREATE', 'create'),'create')]")
        await button[0].click();
        const deleteBtn = await page.$x("//*[contains(translate(text(), 'X', 'x'),'x')]")
    ...
    it("Items is removed when clicking 'x' button", async () => {
        ...
        const button = await page.$x("//*[contains(translate(text(), 'CREATE', 'create'),'create')]")
        await button[0].click();
        expect(await page.$eval('body', el => el.textContent)).toMatch(/Task to delete/i);
        const deleteBtn = await page.$x("//*[contains(translate(text(), 'X', 'x'),'x')]")
        for (let index = 0; index < deleteBtn.length; index++) {
            await deleteBtn[index].click()
        }
        expect(await page.$eval('body', el => el.textContent)).not.toMatch(/Task to delete/i);
    });
});